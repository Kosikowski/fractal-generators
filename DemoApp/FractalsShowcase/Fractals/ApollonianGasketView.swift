...
struct ApollonianGasketView: NSViewRepresentable {
    let iterations: Int

    func makeNSView(context _: Context) -> NSImageView {
        let imageView = NSImageView()
        imageView.image = generateApollonianGasket(iterations: iterations)
        return imageView
    }

    func updateNSView(_ nsView: NSImageView, context _: Context) {
        nsView.image = generateApollonianGasket(iterations: iterations)
    }

    func generateApollonianGasket(iterations: Int) -> NSImage {
        let width = 600
        let height = 600
        ...
        drawCircle(context: context, circle: outerCircle)
        drawCircle(context: context, circle: circle1)
        drawCircle(context: context, circle: circle2)
        drawCircle(context: context, circle: circle3)

        // Recursively fill with smaller circles
        fillGasket(context: context,
                   c1: circle1,
                   c2: circle2,
                   c3: circle3,
                   depth: iterations)
    ...
    private func drawCircle(context: CGContext, circle: Circle) {
        let rect = CGRect(x: circle.x - circle.radius,
    ...
    private func fillGasket(context: CGContext,
                            ...
                            depth: Int)
    {
        ...
        let newCircle = Circle(x: z4_real,
                               y: z4_imag,
                               radius: newRadius)

        if newRadius > 1 {
            drawCircle(context: context, circle: newCircle)
            fillGasket(context: context, c1: c1, c2: c2, c3: newCircle, depth: depth - 1)
            fillGasket(context: context, c1: c2, c2: c3, c3: newCircle, depth: depth - 1)
            fillGasket(context: context, c1: c3, c2: c1, c3: newCircle, depth: depth - 1)
        }
    }
}
