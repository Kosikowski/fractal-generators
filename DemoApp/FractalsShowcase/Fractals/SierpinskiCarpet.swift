 ...
 *
 * The Sierpinski Carpet is a fractal created by recursively removing the center
 * square from a square and repeating this process on the remaining 8 squares.
 *
...
struct SierpinskiCarpetView: View {
    ...
    func sierpinskiCarpetPath(in rect: CGRect, iterations: Int) -> Path {
        var path = Path()
        sierpinskiCarpetRecursive(in: rect, depth: iterations, path: &path)
        return path
    }

    func sierpinskiCarpetRecursive(in rect: CGRect, depth: Int, path: inout Path) {
        if depth == 0 {
            path.addRect(rect)
        } else {
            let w = rect.width / 3
